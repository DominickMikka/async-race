...
class Winners {
  ...
  constructor(root: HTMLElement) {
    this.winnersWrapper = document.createElement('table');
    this.winnersWrapper.classList.add('winners');
    this.bodyPage = document.createElement('div');
    this.bodyPage.classList.add('content');
    this.root = root;
  }

  createWinnersPage(winnerCount: number): void {
    this.bodyPage.innerHTML = '';
    this.bodyPage.insertAdjacentHTML('afterbegin', 
      this.createWinnersLabel(winnerCount) + 
  ...
  createWinnersLabel(winnersCount: number) {
    return `
      <div class='garage-label'>
        Count winners: <strong id='winners-count'>${winnersCount}</strong>
      </div>
    `;
  }
  ...
  createPageLabel(page: number) {
    return `
      <div class='page-label'>
        Current page is: <strong id='page-winners-number'>${page}</strong>
      </div>`;
  }
    ...
    return `
        <thead>
          <tr class='winners-header'>
            <th class='winner-number'>Number</th>
            <th class='winner-car'>Car</th>
            <th class='winner-name'>Name</th>
            <th class='winner-wins'>Wins</th>
            <th class='winner-best-time'>Best time (sec.)</th>
          </tr>
        </thead>
    `;
  }
