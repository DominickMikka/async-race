...
class Garage {
  ...
  createGarage(root: HTMLElement, cars: ICar[]): void {
    ...
    cars.forEach((car: ICar) => {
      root.insertAdjacentHTML('beforeend', this.createCarElement(car));
    });
  }
  ...
  createControls() {
    return `
      <div class='controls'>
        ...
        <button id='start-race'>Race</button> <button id='reset-cars'>Reset</button> <button id='generate-cars'>Generate cars</button>
      </div>
    `;
  }
